...
def process_count_node(node):
    ...
    tree, the more often it is counted. We don't actually have to double
    anything.

    :param node: a count element
    :return: ascii value
    """
    ...
    if not counts:
        return 1
    return sum([
        process_count_node(count) for count in counts
    ])


def get_ascii_char(node):
    """
    Process the contents of a <character> element, and determine its ascii
    value.

    :param node: element tag of the character element
    :return: ascii value
    """
    if len(node.find_all('count')) == 0:
        return 0
    return process_count_node(node)
